...
public class ClangtidyBuildResultEvaluator {
	public Result evaluateBuildResult(final BuildListener listener, int errorsCount, int newErrorsCount,
		...
		if (isErrorCountExceeded(errorsCount, severityEvaluation.getFailureThreshold())) {
			ClangtidyLogger.log(listener, "Setting build status to FAILURE since total number of issues '" + errorsCount
		...
		if (isErrorCountExceeded(newErrorsCount, severityEvaluation.getNewFailureThreshold())) {
			ClangtidyLogger.log(listener,
					"Setting build status to FAILURE since number of new issues '" + newErrorsCount
		...
		if (isErrorCountExceeded(errorsCount, severityEvaluation.getThreshold())) {
			ClangtidyLogger.log(listener, "Setting build status to UNSTABLE since total number of issues '"
					+ errorsCount + "' exceeds the threshold value '" + severityEvaluation.getThreshold() + "'.");
			return Result.UNSTABLE;
		}
		if (isErrorCountExceeded(newErrorsCount, severityEvaluation.getNewThreshold())) {
			ClangtidyLogger.log(listener, "Setting build status to UNSTABLE since number of new issues '"
					+ newErrorsCount + "' exceeds the threshold value '" + severityEvaluation.getNewThreshold() + "'.");
			return Result.UNSTABLE;
		}
