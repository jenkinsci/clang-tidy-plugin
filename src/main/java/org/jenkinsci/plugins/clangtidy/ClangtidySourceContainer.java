...
public class ClangtidySourceContainer {
	...
	public ClangtidySourceContainer(BuildListener listener, FilePath workspace, FilePath scmRootDir,
			List<ClangtidyFile> files) throws IOException, InterruptedException {

		int key = 1;
		for (ClangtidyFile clangtidyFile : files) {
			ClangtidyWorkspaceFile clangtidyWorkspaceFile = getClangtidyWorkspaceFile(listener, workspace, scmRootDir,
					clangtidyFile);
	...
	private ClangtidyWorkspaceFile getClangtidyWorkspaceFile(BuildListener listener, FilePath workspace,
			FilePath scmRootDir, ClangtidyFile clangtidyFile) throws IOException, InterruptedException {
		...
		if (!sourceFilePath.exists()) {
			ClangtidyLogger.log(listener, "[WARNING] - The source file '" + sourceFilePath.toURI()
	...
	private FilePath getSourceFile(FilePath workspace, FilePath scmRootDir, String clangtidyFileName)
